...
class RecognitionWorker(threading.Thread):
    ...
    def run(self):
        ...
        backend = 0
        if str(config.CAMERA_BACKEND).lower() in ("dshow", "directshow"):
        ...
        try:
            fourcc = cv2.VideoWriter_fourcc(*str(config.CAMERA_FOURCC)[:4])
        ...
        while self._running:
            ...
            if idx % config.SKIP_FRAMES == 0:
                ...
                for box in results[0].boxes:
                    try:
                        ...
                        hx1 = max(0, x1 - config.PADDING)
                        hy1 = max(0, y1 - config.PADDING)
                        hx2 = min(frame.shape[1], x2 + config.PADDING)
                        hy2 = min(frame.shape[0], y2 + config.PADDING)

                        crop_bgr = frame[hy1:hy2, hx1:hx2]
                        if crop_bgr.size == 0:
                            continue
                        crop_rgb = cv2.cvtColor(crop_bgr, cv2.COLOR_BGR2RGB)

                        fboxes, probs = mtcnn.detect(crop_rgb)
                        if fboxes is None:
                            continue
                        ...
                        fx1 = max(0, fx1)
                        fy1 = max(0, fy1)
                        fx2 = min(crop_rgb.shape[1], fx2)
                        fy2 = min(crop_rgb.shape[0], fy2)

                        # If MTCNN returns a box that is basically the whole crop, skip it.
                        crop_area = float(crop_rgb.shape[0] * crop_rgb.shape[1])
                        face_area = float(max(0, fx2 - fx1) * max(0, fy2 - fy1))
                        if crop_area > 0 and face_area / crop_area > 0.60:
                            continue

                        face_rgb = crop_rgb[fy1:fy2, fx1:fx2]
                        if face_rgb.size == 0:
                            continue

                        face_rgb_160 = cv2.resize(face_rgb, (160, 160), interpolation=cv2.INTER_LINEAR)
                        face_rgb_112 = cv2.resize(face_rgb_160, (112, 112), interpolation=cv2.INTER_LINEAR)

                        bgr = face_rgb_112[:, :, ::-1].astype(np.float32)
                        bgr = (bgr - 127.5) / 127.5
                        ...
                        if not info.get("matched"):
                            try:
                                ...
                                cv2.imwrite(
                                    fpath,
                                    cv2.cvtColor(face_rgb_160, cv2.COLOR_RGB2BGR),
            ...
            _, jpeg = cv2.imencode(".jpg", out, [cv2.IMWRITE_JPEG_QUALITY, config.JPEG_QUALITY])
